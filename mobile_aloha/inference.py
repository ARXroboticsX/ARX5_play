...
def actions_interpolation(opt, pre_action, actions, stats):
    steps = np.concatenate((np.array(opt.arm_steps_length), np.array(opt.arm_steps_length)), axis=0)
    ...
    for i in range(max_diff_index, post_action.shape[0]):
        step = max(
            [math.floor(math.fabs(result[-1][j] - post_action[i][j]) /
                        steps[j]) for j in range(pre_action.shape[0])])
        inter = np.linspace(result[-1], post_action[i], step + 2)
        result.extend(inter[1:])
    while len(result) < opt.chunk_size + 1:
        result.append(result[-1])
    result = np.array(result)[1:opt.chunk_size + 1]
...
def inference_process(opt, config, ros_operator, policy, stats, t, pre_action):
    ...
    while True and not rospy.is_shutdown():
        ...
        obs['qvel'] = np.concatenate((np.array(puppet_arm_left.velocity),
                                      np.array(puppet_arm_right.velocity)), axis=0)
        obs['effort'] = np.concatenate((np.array(puppet_arm_left.effort),
                                        np.array(puppet_arm_right.effort)), axis=0)
        ...
        if opt.use_actions_interpolation:
            inference_actions = actions_interpolation(opt, pre_action, inference_actions, stats)
...
class RosOperator:
    ...
    def puppet_arm_publish_continuous_thread(self, left, right):
        if self.puppet_arm_publish_thread is not None:
            ...
            self.puppet_arm_publish_thread = None
        self.puppet_arm_publish_thread = threading.Thread(target=self.puppet_arm_publish_continuous,
                                                          opt=(left, right))
    ...
    def init_ros(self):
        ...
        rospy.Subscriber(self.config['camera_config']['original_image']['img_front_topic'],
                         Image, self.img_front_callback, queue_size=1000, tcp_nodelay=True)
        ...
        if self.opt.use_depth_image:
            ...
            rospy.Subscriber(self.config['camera_config']['original_image']['img_front_depth_topic'],
                             Image, self.img_front_depth_callback, queue_size=1000, tcp_nodelay=True)
        ...
        self.puppet_arm_left_publisher = rospy.Publisher(self.config['arm_config']['puppet_arm_left_cmd_topic'],
                                                         JointState, queue_size=10)
        self.puppet_arm_right_publisher = rospy.Publisher(self.config['arm_config']['puppet_arm_right_cmd_topic'],
                                                          JointState, queue_size=10)
